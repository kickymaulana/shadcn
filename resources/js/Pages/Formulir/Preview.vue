...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 md:p-8 pt-4">
        <div class="flex items-center justify-between w-full max-w-[210mm] mx-auto">
            ...
            >
                <Link :href="route('formulirs.show', { formulir: formulir.id })">
            ...
            <Button variant="destructive" size="sm" class="rounded-full shadow-md h-8 text-[10px] font-bold uppercase">
                <a :href="route('pdf.formulir', { formulir: formulir.id })" target="_blank" class="flex items-center">
        ...
        <div class="w-full flex justify-center">
            <Card class="border-none shadow-2xl overflow-x-auto bg-slate-200 flex justify-start md:justify-center py-10 px-4 w-full">
                <div class="bg-white w-[210mm] min-w-[210mm] min-h-[297mm] p-[12mm] shadow-sm text-slate-900 font-sans border border-slate-300 mx-auto relativ">
                    ...
                    <div v-for="(dept, index) in formulir.departemen_terlibat" :key="dept.id" class="mb-8 italic leading-tight">
                        ...
                        <div class="grid grid-cols-3 text-[9px] px-1 mb-2 font-medium">
                            ...
                            <div class="text-right uppercase font-bold">QC: {{ dept.qcUser?.name || dept.qc_user?.name || "-" }} | SPV: {{ dept.spvUser?.name || dept.spv_user?.name || "-" }}</div>
                        </div>
                        ...
                        <div class="border-2 border-slate-900 overflow-hidden">
                            <table class="w-full text-slate-900 border-collapse text-[9px]">
                                ...
                                <tbody>
                                    <tr v-for="(row, idx) in (dept.item_pemeriksaan || [])" :key="idx" class="border-b border-slate-900 last:border-0 h-7">
                    ...
                    <div class="mt-12 grid grid-cols-2 border-2 border-slate-900 italic font-bold uppercase leading-tight">
                        <div class="border-r-2 border-slate-900 flex flex-col">
                            <div class="bg-slate-50 py-1 text-[9px] font-black border-b-2 border-slate-900 text-center text-slate-500 italic">
                                Di Periksa Oleh :
                            </div>
                            <div class="h-32 flex flex-col items-center justify-center gap-1.5 relative px-4 text-center">
                                <template v-if="formulir.pemeriksa">
                                    ...
                                    <span class="text-[7px] text-muted-foreground tracking-widest font-black">Digitally Signed</span>
                                </template>
