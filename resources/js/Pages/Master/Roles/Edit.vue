...
const form = useForm({
    ...
    // Ambil hanya ID permission yang sudah dimiliki
        ...
        <div class="max-w-4xl mx-auto w-full">
            <Card class="border-none shadow-lg bg-card">
                <CardHeader class="pb-8">
                    <CardTitle class="text-2xl font-bold">Edit Jabatan / Role</CardTitle>
                    <CardDescription class="text-sm">
                        Perbarui nama jabatan dan atur hak akses untuk <span class="text-foreground font-semibold">{{ role.name }}</span>.
                    </CardDescription>
                </CardHeader>
                ...
                <form @submit.prevent="submit">
                    <CardContent class="grid gap-6">
                        <div class="grid gap-3">
                            <Label for="name" class="text-sm font-medium leading-none ml-0.5">Nama Jabatan</Label>
                            <Input
                                id="name"
                                v-model="form.name"
                                type="text"
                        ...
                        <div class="pt-6 mt-2 border-t border-dashed">
                            <h3 class="text-sm font-semibold mb-6 text-primary tracking-wide uppercase text-xs">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                <div
                                    ...
                                    :class="form.permissions.includes(permission.id) ? 'border-primary/50 bg-primary/5' : 'border-border'"
                                >
                                    ...
                                    <label class="ml-3 text-sm font-medium leading-none cursor-pointer capitalize group-hover:text-foreground">
                                        {{ permission.name.replace(/-/g, ' ') }}
                                    </label>
                                </div>
                            </div>

                            <p v-if="form.errors.permissions" class="text-xs font-medium text-destructive mt-4">
