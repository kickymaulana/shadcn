  ...
  <div class="flex flex-col gap-6 p-4 md:p-8 pt-4">
    ...
    <div class="max-w-2xl mx-auto w-full">
      <Card class="border-none shadow-lg bg-card">
        <CardHeader class="pb-8"> <CardTitle class="text-2xl font-bold">Edit Profil Pengguna</CardTitle>
          <CardDescription class="text-sm">
            Perbarui informasi akun untuk <span class="text-foreground font-semibold">{{ user.name }}</span>.
            Kosongkan password jika tidak ingin mengubahnya.
          </CardDescription>
        </CardHeader>

        <form @submit.prevent="submit">
          <CardContent class="grid gap-6"> <div class="grid gap-3"> <Label for="name" class="text-sm font-medium leading-none ml-0.5">Nama Lengkap</Label>
              <Input
                id="name"
                v-model="form.name"
                type="text"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> <div class="grid gap-3">
                <Label for="username" class="text-sm font-medium leading-none ml-0.5">Username</Label>
                <Input
                  id="username"
              ...
              <div class="grid gap-3">
                <Label for="email" class="text-sm font-medium leading-none ml-0.5">Email</Label>
                <Input
                  id="email"
                  v-model="form.email"
                  type="email"
            ...
            <div class="pt-6 mt-2 border-t border-dashed">
              <h3 class="text-sm font-semibold mb-6 text-primary tracking-wide uppercase text-xs">Ganti Password (Opsional)</h3>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="grid gap-3">
                  <Label for="password" class="text-sm font-medium leading-none ml-0.5">Password Baru</Label>
                  <Input
                    id="password"
                ...
                <div class="grid gap-3">
                  <Label for="password_confirmation" class="text-sm font-medium leading-none ml-0.5">Konfirmasi Password</Label>
                  <Input
              ...
              <p v-if="form.errors.password" class="text-xs font-medium text-destructive mt-3">{{ form.errors.password }}</p>
            </div>
          </CardContent>

          <CardFooter class="flex justify-end gap-3 border-t bg-muted/20 px-6 py-6 rounded-b-lg">
            <Button variant="outline" type="button" as-child :disabled="form.processing" class="h-10 px-6">
