...
// Logika Hak Akses berdasarkan Role Spatie
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 md:p-8 pt-4">
        <div class="flex items-center justify-between w-full max-w-[210mm] mx-auto">
            ...
            >
            <Link :href="route('tugas.produksi.edit', { departemen_terlibat: props.active_dept_id })">
                ...
                <AlertDialog v-if="!formulir.diperiksa_oleh && canParafPemeriksa()">
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                Berikan paraf digital sebagai <strong>QC Manager</strong>?
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel>Batal</AlertDialogCancel>
                            <AlertDialogAction @click="submitParaf('formulirs.paraf_pemeriksa')">
                                Ya, Paraf
                            </AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                ...
                <AlertDialog v-if="!formulir.disetujui_oleh && formulir.diperiksa_oleh && canParafPenyetuju()">
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                Berikan persetujuan akhir sebagai <strong>Factory Manager</strong>?
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel>Batal</AlertDialogCancel>
                            <AlertDialogAction @click="submitParaf('formulirs.paraf_penyetuju')" class="bg-blue-600">
                                Ya, Setujui
                            </AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                ...
                <div class="bg-white w-[210mm] min-h-[297mm] p-[12mm] shadow-sm text-slate-900 font-sans border border-slate-300 mx-auto">
                    ...
                    <div class="grid grid-cols-2 gap-x-10 gap-y-1 text-[10px] mb-8 border-b-2 border-slate-100 pb-4 italic font-medium">
                        <div class="flex justify-between">
                            ...
                            <span class="flex-1 font-black">{{ sampel.kode_sample }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="w-24 text-slate-500 uppercase font-bold">Customer</span>:
                            <span class="flex-1 font-black">{{ sampel.customer }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="w-24 text-slate-500 uppercase font-bold">Model / Size</span>:
                            <span class="flex-1">{{ sampel.model }} / {{ formulir.size }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="w-24 text-slate-500 uppercase font-bold">Tgl Minta</span>:
                            <span class="flex-1">{{ formatDate(formulir.tanggal_permintaan) }}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="flex-1 font-black underline">{{ formulir.qty_sampel_kirim }} Pcs</span>
                        </div>
                    </div>
                    ...
                    <div v-for="dept in formulir.departemen_terlibat" :key="dept.id" class="mb-8 italic leading-tight">
                        ...
                        <div class="grid grid-cols-3 text-[9px] px-1 mb-2 font-medium">
                            ...
                            <div class="text-right uppercase font-bold">
                                QC: {{ dept.qcUser?.name ?? "-" }} | SPV: {{ dept.spvUser?.name ?? "-" }}
                            </div>
                        </div>
                    </div>

                    <div class="mt-12 grid grid-cols-2 text-center text-[10px] italic font-bold uppercase">
                        <div class="space-y-12">
                            <p>Diperiksa Oleh (QC Manager)</p>
                            <div class="h-10 flex items-center justify-center">
                                <span v-if="formulir.pemeriksa" class="text-primary tracking-widest">[ DIGITAL SIGNATURE: {{ formulir.pemeriksa.name }} ]</span>
                                <span v-else class="text-slate-300 italic">BELUM DIPARAF</span>
                            </div>
                            <p class="underline">{{ formulir.pemeriksa?.name ?? ".........................." }}</p>
                        </div>
                        <div class="space-y-12">
                            <p>Disetujui Oleh (Factory Manager)</p>
                            <div class="h-10 flex items-center justify-center">
                                <span v-if="formulir.penyetuju" class="text-blue-600 tracking-widest">[ DIGITAL SIGNATURE: {{ formulir.penyetuju.name }} ]</span>
                                <span v-else class="text-slate-300 italic">BELUM DISETUJUI</span>
                            </div>
